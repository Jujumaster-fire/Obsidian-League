-- 00: base schema - the original schema.sql + schema_updates.sql, merged.
--
-- Why this file exists: `supabase db push` applies supabase/migrations/* in
-- filename order, and migrations 01..11 all ALTER/extend the base tables
-- (user_roles, teams, fixtures, match_events, tournament_settings, storage).
-- Without it a fresh project fails at 01_add_team_details.sql.
--
-- It mirrors PART 0 of supabase/db-setup.sql, which stays the single-shot
-- source of truth: applying db-setup.sql in the SQL editor and pushing 00..11
-- with the CLI converge on exactly the same schema (every statement is
-- idempotent, so doing both is safe).
--
-- Generated from supabase/_parts/00_base.sql: edit that part, re-run
-- `node scripts/build-db-setup.mjs`, then mirror the change here.
-- ============================================================================
